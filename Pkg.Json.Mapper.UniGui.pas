unit Pkg.Json.Mapper.UniGui;

interface
uses System.JSON, Rest.Json, RTTI, RegularExpressions, TypInfo,
...
type
  ...
  TExporClass = (TArray, TObjectList);
  ...
  TPkgJsonMapper = class
    private
      //[ALTERADO] FTreeView: TTreeView;
      FClasses: TList<TStubClass>;
      FRootClass: TStubClass;
      FUnitName: string;
      FExportClass: TExporClass;
    ...
    protected
      ...
      //[ALTERADO] procedure InternalFormatTreeViewFields(AItem: TTreeViewItem);
      //[ALTERADO] procedure FormatFields(ATreeView: TTreeView);
      //[ALTERADO] procedure InternalVisualize(ATreeViewItem: TTreeViewItem; AClass: TStubClass; AItemStyleLookup: string);
    ...
    public
      ...
      //[ALTERADO] procedure   Visualize(ATreeView: TTreeView; AItemStyleLookup: string);
      property    DestinationUnitName: string read FUnitName write SetUnitName;
      property    ExportClass: TExporClass read FExportClass write FExportClass;
  end;
...
begin
  ...
  try
    ...
    LList.Add('');
    LList.Add('{*******************************************************************************');
    LList.Add('    Generated By   : JsonToDelphiClass - 0.65 ');
...
begin
  ...
  try
    ...
    if FMapper.FExportClass = TExporClass.TArray then
    begin
    ...
    if FMapper.FExportClass = TExporClass.TObjectList then
    begin
      if (FComplexItems.Count > 0) OR (FArrayItems.Count > 0) then
      begin
        LLines.Add(format('constructor %s.Create;', [LClassName]));
        ...
        for LItem in FArrayItems do
        begin
          LString := format('  F%s := TObjectList<%s>.Create();', [LItem.FName, (LItem as TStubContainerField).FFieldClass.Name]);
          LLines.Add(LString);
        end;
        ...
        begin
          for LItem in FArrayItems do
            LLines.Add(format('  %s.free;', [LItem.FieldName]));
        end;
...
begin
  ...
  try
    ...
    if Self.FMapper.ExportClass = TExporClass.TArray then
      if FComplexItems.Count > 0 then
      begin
        LLines.Add('  constructor Create;');
      end;

    if Self.FMapper.ExportClass = TExporClass.TObjectList then
      if (FComplexItems.Count > 0) OR (FArrayItems.Count > 0) then
      begin
        LLines.Add('  constructor Create;');
      end;
...
begin
  ...
  case Self.FParentClass.FMapper.FExportClass of
    TArray: result := format('TArray<%s>', [LSubType]);
    TObjectList: result := format('TObjectList<%s>', [LSubType]);
  end;
end;
